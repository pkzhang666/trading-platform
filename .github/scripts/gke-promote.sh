...
render_manifest() {
  python "${render_script}" \
    ...
    --value "API_STABLE_IMAGE=${1}" \
    --value "API_CANARY_IMAGE=${2}" \
    ...
    --value "WEB_STABLE_IMAGE=${3}" \
    --value "WEB_CANARY_IMAGE=${4}" \
    --value "WEB_STABLE_WEIGHT=100" \
    --value "WEB_CANARY_WEIGHT=0" \
    --value "WEB_CANARY_REPLICAS=0" \
    --value "ADMIN_STABLE_IMAGE=${5}" \
    --value "ADMIN_CANARY_IMAGE=${6}" \
...
api_canary_image="$(current_image api-canary)"
web_canary_image="$(current_image web-canary)"
admin_canary_image="$(current_image admin-canary)"

render_manifest \
  "${api_canary_image:-$api_stable_image}" "${api_canary_image:-$api_stable_image}" \
  "${web_canary_image:-$web_stable_image}" "${web_canary_image:-$web_stable_image}" \
  "${admin_canary_image:-$admin_stable_image}" "${admin_canary_image:-$admin_stable_image}"
