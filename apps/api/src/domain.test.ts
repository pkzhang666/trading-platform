...
test("matches crossing orders and records a trade", () => {
  ...
  try {
    ...
    const trades = platform.listRecentTrades("BTC/USD");
...
test("withdrawals freeze funds until admin review", () => {
  ...
  try {
    ...
    const withdrawal = platform.requestWithdrawal(trader.user.id, "USD", 100, "wallet-addr-1");
    assert.equal(withdrawal.status, "pending");

    const pendingDashboard = platform.getDashboard(trader.user.id);
    const pendingUsdt = pendingDashboard.balances.find((item) => item.asset === "USD");
    ...
    const finalUsdt = finalDashboard.balances.find((item) => item.asset === "USD");
    assert.ok(finalUsdt);
